    ...
    <>
      ...
      {images.map((image, index) => (
        <React.Fragment key={image.src}>
          ...
          >
            {alt ?? (
              <p className="text-heading-sm">Search All Around The World !</p>
            )}
            {"coords" in image && (
              <Link
                className="relative text-heading-md underline transition-all hover:scale-110 hover:animate-pulse"
