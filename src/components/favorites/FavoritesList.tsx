...
export function FavoritesList({ favorites }: { favorites: FavoritesT }) {
  ...
  return Object.keys(favorites).map((key, index) => (
    <motion.li
      ...
      className="weather-heading max-lg:max-w-30 flex items-center gap-x-1 overflow-ellipsis rounded-l-full border-b-2 border-l-2 border-t-2 border-gray-200 bg-blue-light px-2 text-gray-100 dark:border-gray-600 dark:bg-gray-500"
    >
      <Link
        to={`/weather/${key}`}
        className="user-drag-none flex h-full w-full items-center whitespace-pre-line py-1"
      >
        {favorites[key]}
      </Link>
      <motion.button
