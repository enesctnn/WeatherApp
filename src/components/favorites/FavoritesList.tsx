...
export function FavoritesList({ favorites }: { favorites: string[] }) {
  ...
  return favorites.map((favorite, index) => (
    <motion.li
      ...
      className="px-2 bg-blue-light flex overflow-ellipsis rounded-l-full text-gray-100 weather-heading items-center gap-x-1 max-lg:max-w-30 border-t-2 border-l-2 border-b-2 md:border-t-4 md:border-l-4 md:border-b-4 border-gray-600"
    >
      <Link
        to={`/weather/${favorite}`}
